...
def recordAudio(seconds:int=5,Fs:int=44100,audio_path:str="./audioParsing/output.wav"):
    ...
    recording = sd.rec(5*44100, samplerate=44100, channels=2)
    sd.wait()  
...
def recordAudioToSpectogram(audio_path,image_path):
    recordAudio(audio_path=audio_path)
    Fs,data = getAudioData(audio_path=audio_path)
    return getSpectogram(data,44100,image_path)
